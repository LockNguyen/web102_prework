...
const displayStr = 
    `A total of $${GAMES_JSON.reduce( (total, game) => {
        return total + game.pledged;
    }, 0).toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. Currently, ${unfundedGamesCount} ${unfundedGamesCount === 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`;
...
const secondTopPledgeGame = document.createElement("p");
secondTopPledgeGame.innerHTML = secondGame.name;
secondGameContainer.appendChild(secondTopPledgeGame);